...
function addDataToSheet(email, password, displayname, bio, biolink) {
  ...
  sheet.getRange(lastRow + 1, 5).setValue(bio); //Kuch Bhi dalo koi na padta
  sheet.getRange(lastRow + 1, 6).setValue(biolink);
  sheet.getRange(lastRow + 1, 7).setValue(tstamp); //Wo kartein hai na Account created in Sept 2014 Wo wala scene
